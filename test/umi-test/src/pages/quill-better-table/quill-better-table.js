...
class BetterTable extends Module {
  static register() {
    ...
    Quill.register(TableCellUl, true);
    Quill.register(TableCellUlLi, true);
  ...
  constructor(quill, options) {
    ...
    // 点击事件 目前功能只有 显示 table tools 
    // handle click on quill-better-table
    this.quill.root.addEventListener('click', (evt) => {
      // bugfix: evt.path is undefined in Safari, FF, Micro Edge
      const path = getEventComposedPath(evt)
      // path 是一个数组 包含了从 window 到当前节点中所有 dom
    ...
    // prevent user hits backspace to delete table cell 防止用户按了 backspace 删除表格单元格
    const KeyBoard = quill.getModule('keyboard')
    // 添加了回调函数 就不会删除 表格单元格 
    // 而 返回 false 表示不会删除
    // 在表格外删除时 剩最后一个元素就不会删除 以免光标落入表格里
    ...
    //since only one matched bindings callback will excute. 因为只有一个匹配的绑定回调将执行
    // expected my binding callback excute first 期望我的绑定回调先执行
    // I changed the order of binding callbacks 我更改了绑定回调的顺序
